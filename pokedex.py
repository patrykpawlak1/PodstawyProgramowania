...
def wyswietl_pokemony_wg_typu(pokedex, text_widget ):
    ...
    if not pokemony_wg_typu:
        text_widget.insert(tk.END, "Brak pokemonów według kryteriów.\n")
    else:
...
def dodaj_grafike_pokemona(pokedex, text_widget):
    ...
    nazwa_pokemona = simpledialog.askstring("Dodaj grafikę pokemona", "Podaj nazwę pokemona, dla którego chcesz dodać grafikę:")
    ...
    if not pokemon:
        text_widget.insert(tk.END, f"{nazwa_pokemona} nie istnieje w pokedexie.\n")
        return

    sciezka_do_grafiki = filedialog.askopenfilename(title="Wybierz grafikę pokemona", filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.gif")])

    try:
        obraz = Image.open(sciezka_do_grafiki)
    ...
    text_widget.insert(tk.END, f"Grafika dla {nazwa_pokemona} została dodana!\n")
...
def Pokedex():
    ...
    for nazwa, funkcja in przyciski.items():
        przycisk = tk.Button(root, text=nazwa, command=funkcja)
        przycisk.pack(pady=4)
